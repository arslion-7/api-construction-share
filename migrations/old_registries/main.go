...
func main() {
	...
	fmt.Println("Starting migration of old registries from MySQL to PostgreSQL...")
	...
	rows, err := mysqlDB.Query("SELECT * FROM mainpayly")
	if err != nil {
	...
	for rows.Next() {
		...
		// Upsert into PostgreSQL (insert or update based on t_b)
		var existingRecord models.OldRegistry
		result := PostgresDB.Where("t_b = ?", oldRegistry.TB).First(&existingRecord)
			...
			if result.Error == gorm.ErrRecordNotFound {
				...
				if err := PostgresDB.Create(&oldRegistry).Error; err != nil {
					...
					if processedCount%100 == 0 {
						log.Printf("Processed %d records (inserted: %d, updated: %d)...", processedCount, insertedCount, updatedCount)
					}
				}
			} else {
				log.Printf("Failed to check existing record (TB: %d): %v", oldRegistry.TB, result.Error)
			}
		} else {
			// Record exists, update it
			if err := PostgresDB.Model(&existingRecord).Updates(oldRegistry).Error; err != nil {
				log.Printf("Failed to update old registry (TB: %d): %v", oldRegistry.TB, err)
			} else {
				processedCount++
				updatedCount++
				if processedCount%100 == 0 {
	...
	log.Printf("Migration completed successfully! Processed %d records (inserted: %d, updated: %d).", processedCount, insertedCount, updatedCount)
}
